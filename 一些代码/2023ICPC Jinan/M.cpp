...
int main(){
    ...
    std::vector<int> sta;
    for (int i = 0; i < n; i++){
    	while (sta.size() > 1 && !pointOnLineLeft(p[i], Line(p[sta[sta.size() - 2]], p[sta.back()]))){
    		sta.pop_back();
    	}
    	sta.push_back(i);
    }
    int j = sta.size();
    for (int i = n - 2; i >= 0; i--){
    	while (sta.size() > j && !pointOnLineLeft(p[i], Line(p[sta[sta.size() - 2]], p[sta.back()]))){
    		sta.pop_back();
    	}
		sta.push_back(i);
    }
    ...
    for (int i = 0; i < n; i++){
    	...
    	for (int j = 0; j < n - 1; j++){
    		// std::cout << pos[j] << " \n"[j == n - 2];
    		int a = pos[j], b = pos[(j + 1) % (n - 1)];
    		if (!map.count(a) || !map.count(b)) continue;
    		if (std::abs(map[a] - map[b]) == 1 || std::abs(map[a] - map[b]) == sta.size() - 1) ans += 1;
    	}
    	// std::cout << "###\n";
    }
